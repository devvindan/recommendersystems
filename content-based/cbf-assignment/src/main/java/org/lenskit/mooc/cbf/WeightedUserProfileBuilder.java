...
public class WeightedUserProfileBuilder implements UserProfileBuilder {
    ...
    public Map<String, Double> makeUserProfile(@Nonnull List<Rating> ratings) {
        ...
        for (Rating r: ratings) {
                ...
                Map<String, Double> vec = model.getItemVector(r.getItemId());

                for (String tag : vec.keySet()){
                    double currentTag = profile.containsKey(tag) ? profile.get(tag) : 0;
                    profile.put(tag, currentTag + vec.get(tag) * (r.getValue() - mean));
                }
        }
