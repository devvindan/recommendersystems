...
public class TFIDFModelProvider implements Provider<TFIDFModel> {
    ...
    public TFIDFModel get() {
        ...
        for (long item : items) {
            ...
            for (Entity tagApplication : dao.query(TagData.ITEM_TAG_TYPE)
                ...
                if (!work.containsKey(tag)){
                    double currentGlobalNum = docFreq.containsKey(tag) ? docFreq.get(tag) : 0;
                    docFreq.put(tag, currentGlobalNum + 1);
                }

                double currentNum = work.containsKey(tag) ? work.get(tag) : 0;
                work.put(tag, currentNum + 1);
        ...
        for (Map.Entry<Long, Map<String, Double>> entry : itemVectors.entrySet()) {
            ...
            // transform TF to TF-IDF vector, compute Euclidian norm
            double euclidianNorm = 0;

            for (String tag : tv.keySet()){
                tv.put(tag, tv.get(tag) * docFreq.get(tag));
                euclidianNorm += Math.pow(tv.get(tag), 2);
            }

            euclidianNorm = Math.sqrt(euclidianNorm);

            for (String tag : tv.keySet()){
                tv.put(tag, tv.get(tag) / euclidianNorm);
            }
