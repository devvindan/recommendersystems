...
public class AssociationItemBasedItemRecommender extends AbstractItemBasedItemRecommender {
    ...
    private ResultList recommendItems(int n, long refItem, LongSet candidates) {
        ...
        for (long candidate : candidates){
            double score = model.getItemAssociation(refItem, candidate);
            results.add(Results.create(candidate, score));
        }

        Comparator<Result> descendingAssociationComporator = new Comparator<Result>() {
            @Override
        ...
        Collections.sort(results, descendingAssociationComporator);
