...
public class MeanItemBasedItemRecommender extends AbstractItemBasedItemRecommender {
    ...
    private ResultList recommendItems(int n, LongSet items) {
        ...
        for (long item : items){
            try {
                double meanRating = model.getMeanRating(item);
                Result result = Results.create(item, meanRating);
                results.add(result);
        ...
        Comparator<Result> descendingMeanComporator = new Comparator<Result>() {
            @Override
        ...
        Collections.sort(results, descendingMeanComporator);
